...
MMproducer::MMproducer(const edm::ParameterSet &iConfig) :
...
void MMproducer::produce(edm::Event &iEvent, const edm::EventSetup &iSetup) {
   ...
   if (priVtxs->begin() != priVtxs->end()) thePrimaryV = reco::Vertex(*(priVtxs->begin()));
   ...
   for (pat::MuonCollection::const_iterator  it = muons->begin(), itend = muons->end(); it != itend; ++it) {
	 ...
	 if (!dimuonSelection_(mmCandidate)) continue;                   // ---- apply the dimuon cut ----

	 // ---- fit vertex using Tracker tracks (if they have tracks) ----
	 float vProb = -1;
	 if (it->track().isNonnull() && it2->track().isNonnull()) {      // build the dimuon secondary vertex
	    ...
	    if (myVertex.isValid()) {
	       ...
	       if (resolveAmbiguity_) {                                  // find the closest PV in z, to the extrapolate MM to beam axis
		  float minDz = 999999.;
		  ...
		  for (reco::VertexCollection::const_iterator itv = priVtxs->begin(), itvend = priVtxs->end(); itv != itvend; ++itv) {		//找到最近的z方向轨迹 匹配为mu子对
   ...
   if (i_mm>1) std::sort(oniaOutput->begin(), oniaOutput->end(), vPComparator_);  // sort by vProb if any
