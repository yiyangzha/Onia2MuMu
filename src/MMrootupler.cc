...
MMrootupler::MMrootupler(const edm::ParameterSet & iConfig):
...
void MMrootupler::analyze(const edm::Event & iEvent, const edm::EventSetup & iSetup) {
   ...
   if (priVtxs.isValid()) numPrimaryVertices = (int) priVtxs->size();
   ...
   if ( (isMC_ || only_gen_) && packed.isValid() && pruned.isValid() ) {
	 ...
	 if ( ((abs(aonia->pdgId()) == pdgid_) || (abs(aonia->pdgId()) == 100000+pdgid_) || (abs(aonia->pdgId()) == 200000+pdgid_))  && (aonia->status() == 2) ) {
      ...
      if ( ! dimuon_pdgId ) std::cout << "MMrootupler: does not found the given decay " << run << "," << event << std::endl; // sanity check
   }  // end if isMC
   ...
   if ( ! only_gen_ ) { // we will look for dimuons
   ...
   if ( !already_stored ) {  // we have to make sure, we are not double storing a combination
      if ( !only_gen_ ) {
