...
MMgen::MMgen(const edm::ParameterSet &iConfig) : genCands_(consumes<reco::GenParticleCollection>(iConfig.getParameter<edm::InputTag>("src"))),
...
void MMgen::analyze(const edm::Event &iEvent, const edm::EventSetup &iSetup)
{
   ...
   // sanity check
   if (!ncombo)
      std::cout << "MMgen: Decay not found [" << iEvent.id().run() << "," << iEvent.id().event() << "]" << std::endl;
   // else onia_tree ->Fill();
}
